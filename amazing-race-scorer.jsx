...
export default function App() {
  ...
  async function saveSeason() {
    ...
    try {
      ...
      const [savedSeason] = await db.saveSeason({
    ...
    <div style={css.app}>
      ...
      <div style={{padding:"14px 14px 80px"}}>
        ...
        {tab==="picks" && players.map(pl=>(
          <div key={pl.id} style={css.card}>
            <div style={{fontWeight:700,fontSize:16,marginBottom:10}}>{pl.name}</div>
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:1,background:"#0f1420",borderRadius:10,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"#f1c40f",fontWeight:700,letterSpacing:1,marginBottom:2}}>BLIND</div>
                <div style={{fontSize:13,fontWeight:600}}>{getTeam(pl.blindTeam)?.nickname||<span style={{color:"#475569"}}>Not set</span>}</div>
              </div>
              <div style={{flex:1,background:"#0f1420",borderRadius:10,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"#94a3b8",fontWeight:700,letterSpacing:1,marginBottom:2}}>PICKED</div>
                <div style={{fontSize:13,fontWeight:600}}>{getTeam(pl.pickedTeam)?.nickname||<span style={{color:"#475569"}}>Not set</span>}</div>
              </div>
            </div>
            {pl.cityGuess&&<div style={{fontSize:12,color:"#64748b"}}>🌍 <span style={{color:"#e2e8f0"}}>{pl.cityGuess}</span></div>}
          </div>
        ))}
      </div>
      ...
      {modal?.type==="buyback"&&modalPlayer&&(
        <Sheet title="BUYBACK" onClose={()=>setModal(null)}>
          ...
          <button onClick={()=>{applyBuyback(modal.playerId,false);setModal(null);}}
            style={{...css.btn("d"),marginBottom:10}}>One eliminated — −10pts</button>
          <button onClick={()=>{applyBuyback(modal.playerId,true);setModal(null);}}
